...
def dag_bigquery_to_huggingface():
    ...
    def register_dataset_to_huggingface(
        ...
        df_sample = pd.read_csv(io.BytesIO(blob_content), sep=",", nrows=1000)

        with io.BytesIO() as byte_stream:
            df_sample.to_csv(byte_stream, index=False)
            byte_stream.seek(0)

            api.upload_file(
                path_or_fileobj=byte_stream,
                path_in_repo="train_sample.csv",
                repo_id=f"{hf_username}/{dataset_repo_name}",
                repo_type="dataset",
                token=hf_token,
            )

    gcs_bucket_name = "mlops-airflow-brad"
